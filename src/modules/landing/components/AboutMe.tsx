...
export function AboutMe(props: { sectionId: string }) {
  ...
  const elapsedYears = new Date(Date.now()).getFullYear() - startDate.getFullYear()

  return (
    <div id={props.sectionId} className="scroll-mt-20 w-full max-w-[70ch] mx-auto">
      <Frame className="space-y-6 font-mono lowercase backdrop-blur">
        <p>
          I'm Aziz; a{' '}
          <span className="text-[1.375rem] text-glow-sm text-shadow-primary font-lowres">web developer</span> with a
          passion for making things people may possibly one day want to use.
        </p>
        ...
        <p>I enjoy reading & making cool components and animations in my spare time.</p>
      </Frame>
    </div>
  )
}
